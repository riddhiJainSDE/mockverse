...
function RecordAnsSection({mockInterviewQuestion,activeQuestionIndex,interviewData}) {
    ...
    const UpdateUserAnswer = async()=>{
        ...
        console.log('Feedback JSON:', mockJsonResp);
        ...
        if (resp) {
            toast('User answer has been successfully recorded');
            setUserAnswer('');
            setResults([]);
        }
        setResults([]);
    ...
    return (
        <div className='flex items-center justify-center flex-col'>
            <div className='flex flex-col mt-20 justify-center items-center bg-black rounded-lg p-5 relative'>
                <Image src={'/webcam1.png'} width={200} height={200} className='absolute' />
                <Webcam
                    style={{
                        height: 300,
                        width: '100%',
            ...
            <Button 
                disabled = {loading} variant="outline" className="my-10" onClick={StartStopRecording}>
                {isRecording ? (
